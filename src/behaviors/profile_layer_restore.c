 ...
 *
 * Deep sleep clears volatile layer state.
 * Restore the intended layer state from the saved BLE profile after boot.
 *
 * BLE0 -> default layer (C hold = CTRL)
 * BLE1 -> Apple layer + Draw layer (iPad)
 * BLE2 -> Apple layer (iPhone)
 * BLE3 -> Apple layer (MacBook)
 * BLE4 -> default layer (C hold = CTRL)
 */
